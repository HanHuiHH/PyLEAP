...
ResultPath = "./Results/Simulation/OATSimulation" + time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime()) + ".xlsx"  # 结果保存路径
...
if __name__ == '__main__':
    ...
    """
        主要模拟
    """
    params_list = [[0.5] * problem['num_vars']] * problem['num_vars']
    for iter in range(problem['num_vars']):

        params_list[iter][iter] = params_list[iter][iter] + 0.5
        params = params_list[iter]
        ...
        params_list[iter][iter] = params_list[iter][iter] - 1
        params = params_list[iter]
        AllRecords = simulation(
            leap=leap,
            AllRanges=GetValueRange(FilePath=ImportPath,
